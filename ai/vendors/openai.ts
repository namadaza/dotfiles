...
import type { AskOptions, ChatInstance, Vendor, VendorConfig } from "../gen-ai";
...
export class OpenAIChat implements ChatInstance {
  ...
  async ask(
    ...
    if (this.vendor === "openai") {
      return this.askViaResponsesAPI({
        options,
        wantStream,
        mergedOpenAIConfig,
      });
    ...
    return this.askViaChatCompletions({
      options,
      wantStream,
      mergedOpenAIConfig,
    });
  ...
  private async askViaChatCompletions({
    ...
    if (wantStream) {
      ...
      const resp: any = await (this.client.chat.completions.create as any)(
        params
      );
