...
const SUPPORTED_VENDORS = new Set<Vendor>([
  "openai",
  "anthropic",
  "google",
  "openrouter",
...
async function getToken(vendor: string): Promise<string> {
  ...
  try {
    ...
    console.error(
      `Error reading ${vendor}.token file:`,
...
export function resolveModelSpec(spec: string): ResolvedModelSpec {
  ...
  const [vendorRaw, modelRaw, thinkingRaw] = parts as [
    string,
    string,
    string | undefined
  ];
...
function mapThinkingToAnthropic(
  ...
  const budget =
    thinking === "low" ? 2048 : thinking === "medium" ? 4096 : 8192;
  return {
    thinking: {
...
export async function GenAI(modelSpec: string): Promise<ChatInstance> {
  ...
  const vendorConfig = buildVendorConfig(
    resolved.vendor,
    resolved.model,
    resolved.thinking
  );
