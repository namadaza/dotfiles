...
- Push the feature branch and create a PR with a summary statement. Format for title should be `feat(app-name): short description`, or `fix(app-name): short description`
...
Use agent-browser during step 8b to capture clean (non-annotated) screenshots of each new or changed page:
...
Derive the screenshot folder name from the spec filename by dropping the `todo-` prefix and `.md` extension. For example, spec `.claude/specs/todo-2026-03-16-refactor-new-campaign-flow.md` → screenshots go in `docs/2026-03-16-refactor-new-campaign-flow/screenshots/`.

1. Copy screenshots into the docs folder:
   ```bash
   mkdir -p docs/<spec-name>/screenshots
   cp /tmp/*.png docs/<spec-name>/screenshots/
   ```

2. Commit and push:
   ```bash
   git add docs/<spec-name>/screenshots/
   git commit -m "docs: add UI screenshots from browser validation"
   git push
   ```

3. Update the PR body to include a `## Screenshots` section with image references. **Use `gh api` with `--field body=@file`** because `gh pr edit` fails on repos with deprecated Projects Classic:
   ```bash
   ...
   ### Page Name
   ![Page Name](https://github.com/<org>/<repo>/blob/<branch>/docs/<spec-name>/screenshots/page-name.png?raw=true)
   ...
   Image URL format: `https://github.com/<org>/<repo>/blob/<branch>/docs/<spec-name>/screenshots/<filename>.png?raw=true`

   **Important**: Screenshots must be committed and pushed before the URLs will resolve. Always push the screenshots commit before updating the PR body.
